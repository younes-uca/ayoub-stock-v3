...
@Component({
  selector: 'app-permutation-item-edit-admin',
  templateUrl: './permutation-item-edit-admin.component.html'
})
export class PermutationItemEditAdminComponent extends AbstractEditController<PermutationItemDto, PermutationItemCriteria, PermutationItemAdminService>   implements OnInit {
   ...
   public async openCreatePermutation(permutation: string) {
        const isPermistted = await this.roleService.isPermitted('Permutation', 'edit');
        if (isPermistted) {
             this.permutation = new PermutationDto();
